...
<option value=""><?php echo t('全部');?></option>
...
$.ajax({
url: "<?php echo u('system/attachment/librarydata');?>",
data:{page:page+1,pagesize:pagesize, type: '<?php echo $type;?>', folderid:folderid},
dataType:'json',
...
html += '<div class="fileitem file clearfix" id="'+file.id+'" data-name="'+file.name+'" data-url="'+file.url+'" data-size="'+file.size+'" data-ext="'+file.ext+'" title="<?php echo t('名称');?> : '+file.name+'<br/>' + '<?php echo t('大小');?> : '+ zotop.formatsize(file.size) +'<br/> ' + (file.width > 0 ? '<?php echo t('宽高');?> : '+file.width+'px × '+file.height+'px' : '') +'">';
...
html += '<div class="action"><a class="delete" title="<?php echo t('删除');?>"><i class="icon icon-delete"></i></a></div>';
html += '</div>';
});
...
prev_text : "<?php echo t('前页');?>",
next_text : "<?php echo t('下页');?>",
...
//删除操作
$.get("<?php echo u('system/attachment/delete');?>",{id : $item['attr']('id')}, function(msg){
if( msg.state ){
