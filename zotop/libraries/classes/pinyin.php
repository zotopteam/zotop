...
class pinyin
{
	 ...
	 * @return string
	 */
	public static function get($str, $s=' ', $utf8 = true)
	{
		//参数分析
		if ( !$str ) return false;

		//编码转换
		$str = ($utf8 == true) ? pinyin::encoding('utf-8', 'gbk', $str) : $str;
		$num = strlen($str);
		...
		for ($i=0; $i<$num; $i++)
		{
			...
			$pinyin .= pinyin::ascii($temp, $s);
		}

		$pinyin = str_replace(' ', $s, $pinyin);
		$pinyin = preg_replace("/[[:punct:]]/",$s,$pinyin);
		$pinyin = $s ? preg_replace('#['.$s.$s.']+#', $s, $pinyin) : $pinyin;
		$pinyin = trim($pinyin, $s);

		//输出的拼音编码转换.
		return ($utf8==true) ? pinyin::encoding('gbk', 'utf-8', $pinyin) : $pinyin;
	}
	 ...
	 * @return string
	 */
	protected static function ascii($num, $s)
	{
		if ($num>0 && $num<160)
		{
			return chr($num);
		...
		{
			$total =sizeof(pinyin::$lib)-1;

			for($i=$total; $i>=0; $i--)
			{
				if ( pinyin::$lib[$i][1]<=$num ) break;
			}

			return $s.pinyin::$lib[$i][0].$s;
		}
	}
	 ...
	 * @param string $to		转换后编码
	 * @param string $str		待转换字符串
	 * @return string
	 */
	protected static function encoding($from, $to, $str)
	{
		if(is_string($str) )
		{
