...
(function($){
	$.fn.region = function(options) {
		return this.each(function(){
			...
			//初始化全部select控件
			$.each(options.params,function(name,value){
				select[name] = $('<select name="'+name+'" class="select region" style="margin-right:5px;width:150px"><option value="">'+ options.empty +'</option></select>').appendTo(_this);
			});
