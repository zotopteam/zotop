 ...
 */
class database_controller_schema extends admin_controller
{
	public $config;
	public $db;
	...
	public function __init()
	{
		...
		if ( $_GET['databaseconfig'] )
		{
			$databaseconfig = $_GET['databaseconfig'];

			zotop::cookie('databaseconfig', $databaseconfig);
		}

		if ( empty($databaseconfig) and zotop::cookie('databaseconfig') )
		{
    ...
    public function action_operate($tablename, $operation)
    {
		if ( $post = $this->post() )
		{
			...
			if ( $result )
			{
				return $this->success(t('%s成功',$post['operation']),u("database/schema/{$tablename}"));
			}
    ...
    public function action_addfield($tablename)
    {
		...
		if ( $post = $this->post() )
		{
			...
			{
				return $this->success(t('%s成功',t('新建字段')),u("database/schema/{$tablename}"));
			}
		...
		$this->assign('data',$data);
		$this->display('database/schema_post.php');
    }
    ...
    public function action_editfield($tablename, $field)
    {
		...
		if ( $post = $this->post() )
		{
			...
			{
				return $this->success(t('%s成功',t('编辑字段')),u("database/schema/{$tablename}"));
			}
    ...
    public function action_dropField($table,$field)
    {
		if ( $post = $this->post() )
		{
			...
			{
				return $this->success(t('%s成功',t('删除')) ,u("database/schema/{$table}"));
			}
