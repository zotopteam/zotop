...
(function ($){
	...
	*/
		...
		button:function(args, callback) {
            ...
            if (typeof args === "string") {
                ...
                $.each(args, function(i, val) {
					...
					html += '<button type="button" data-id="' + val.id + '" '+ (val.disabled ? " disabled" :"") + (val.autofocus ? ' autofocus  class="btn btn-highlight"' :'class="btn"') +'>' + val.value + '</button>';
                });
            }
	...
	$.msg = function(msg){
		...
		if( typeof(msg.state) == 'boolean' ) msg.state = msg.state ? 'success' : 'error';
	...
	$.success = function(content, onclose){
		return $.msg({state: 'success',	time: 2, content: content, onclose: onclose});
	}
