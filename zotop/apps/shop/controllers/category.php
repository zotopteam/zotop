 ...
 */
class shop_controller_category extends admin_controller
{
	protected $category;
    ...
    public function action_add($parentid=0)
    {
		if ( $post = $this->post() )
		{
			if ( $id = $this->category->add($post) )
			{
				...
				return $this->success(t('操作成功'),u("shop/category/index/{$post['parentid']}"));
			}
		...
		{
			$data['parentid'] = 0;
			$data['settings'] = array('template_index' => 'shop/index.php','template_list' => 'shop/list.php','template_detail' => 'shop/detail.php');
		}
		...
		$this->assign('parents',$parents);
		$this->display("shop/category_post.php");
    }
