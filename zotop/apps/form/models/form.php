...
class form_model_form extends model
{
	...
	public function add($data)
	{
		...
		// 检查数据表是否存在
		if ( $this->db->existsTable($data['table']) )
		{
		...
		if ( $id = $this->insert($data) )
		{
			...
			// 创建并缓存
			if ( $this->db->createTable($data['table'], $schema) )
			{
				$this->cache(true);
				return $id;
			}
	...
	public function edit($data, $id)
	{
		...
		// 尝试修改表注释
		$this->db->commentTable($data['table'], $data['name']);

		// 更改数据表名称
		if ( $this->get($id,'table') != $data['table'] )
		{
			// 检查数据表是否存在
			if ( $this->db->existsTable($data['table']) ) return $this->error(t('数据表名已经存在'));

			if ( $this->db->renameTable($this->get($id,'table'), $data['table']) == false )
			{				
				unset($data['table']);
			}
		}
